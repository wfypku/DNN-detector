function [all_faces, all_points, all_weights, all_scales, all_sub_scales] = DetectFacesWithSPPParallel(imagelist, rpn_model, pyramid_scale, thr, gpu_id)

if (length(gpu_id) == 1)
    [all_faces, all_points, all_weights, all_scales, all_sub_scales] = DetectFacesWithSPP(imagelist, rpn_model, pyramid_scale, thr, gpu_id);
else
    n = length(imagelist);
    all_faces = cell(n, 1);
    all_points = cell(n, 1);
    ...
    parfor i = 1:gpu_num
        [all_faces_parts{i}, all_points_parts{i}, all_weights_parts{i}, all_scales_parts{i}, all_sub_scales_parts{i}]...
            = DetectFacesWithSPP(imagelist(i:gpu_num:n), rpn_model, pyramid_scale, thr, gpu_id(i));
    end
    for i = 1:gpu_num
